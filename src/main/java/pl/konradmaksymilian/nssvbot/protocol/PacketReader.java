...
public class PacketReader {
    ...
    private Packet readCompressedPacket(int length, ReadField<Integer> uncompressedDataLength) 
    ...
    private Packet readNotCompressedPacket(int length) throws IOException {
        byte[] data = in.readNBytes(length);
        return buildPacket(data);
    }
    
    private Packet buildPacket(byte[] data) throws IOException {
        var buffer = new ByteArrayInputStream(data);
        try (var dataStream = new DataInputStream(buffer)) {
            var packetId = VarIntLongConverter.readVarInt(dataStream);
            if (state == State.LOGIN || state == State.PLAY) {
                return buildLoginPacket(packetId.getValue(), dataStream);
            } else if (state == State.PLAY) {
                //buildPlayPacket(packetId, dataStream);
                return null;
            } else {
                throw new UnexpectedPacketException("The packet with ID: " + packetId + " should not have arrived at the"
                        + " moment");
            }
        }
    }
    
    private Packet buildLoginPacket(int id, DataInputStream data) throws IOException  {
        if (id == PacketName.DISCONNECT_LOGIN.getId()) {
            return PacketBuilder.disconnectLogin(data);
        } if (id == PacketName.LOGIN_SUCCESS.getId()) {
            return PacketBuilder.loginSuccess(data);
        } else if (id == PacketName.SET_COMPRESSION.getId()) {
            return PacketBuilder.setCompression(data);
        } else {
            throw new UnrecognizedPacketException("Cannot recognise the packet - state: handshaking, packet ID: " + id);
        }
    }
}
