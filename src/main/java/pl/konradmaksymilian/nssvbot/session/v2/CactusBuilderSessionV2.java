...
public class CactusBuilderSessionV2 extends MovableSession {
    ...
    public CactusBuilderSessionV2(ConnectionManager connection, Timer timer) {
    ...
    private void onBlockChange(BlockChangePacket packet) {
        ...
        if (builderStatus.equals(BuilderStatusV2.BUILDING) &&
                packet.getPosition().getY() == (int) feetY - 3 && packet.getPosition().getX() == currentX && packet.getPosition().getZ() == currentZ) {
    ...
    private void moveToBuild() {
        ...
        float newYaw;
        float newPitch;
        if (isOdd() && currentZ == FIRST_Z) {
            newX = currentX - 0.5;
            newZ = currentZ + 2.5;
            newYaw = -159.8f;
            newPitch = 63.1f;
        } else if (!isOdd() && currentZ == LAST_Z) {
            newX = currentX + 0.5;
            newZ = currentZ - 0.5;
            newYaw = -0.1f;
            newPitch = 69.0f;
        ...
        System.out.println("current " + currentX + " " + currentZ);
    }
    ...
    private void place() {
        ...
        System.out.println("place " + currentX + " " + (int) (feetY - 3) + " " + currentZ + " " + cursorX + " " + cursorZ);
    }

    private void next() {
        if ((isOdd() && currentZ == LAST_Z) || (!isOdd() && currentZ == FIRST_Z)) {
            currentX += 2;
        } else {
            if (currentX % 2 == 0) {
                currentX++;
